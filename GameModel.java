...
public class GameModel {
  ...
  */
  public GameModel(int width, int heigth, int numberOfMines) {
    ...
    //creating an instance of the DotInfo class and populating it with mines at random locations
    ...
    for (int i = 0; i < numberOfMines; i++) {
      ...
      if(!(model[randomHeight][randomWidth].isMined())) {
        ...
        if(randomWidth-1>=0){
          model[randomHeight][randomWidth-1].setNeighbooringMines(model[randomHeight][randomWidth-1].getNeighbooringMines()+1);
        }
        if(randomWidth+1<widthOfGame){
          model[randomHeight][randomWidth+1].setNeighbooringMines(model[randomHeight][randomWidth+1].getNeighbooringMines()+1);
        }
        if(randomHeight-1>=0){
          model[randomHeight-1][randomWidth].setNeighbooringMines(model[randomHeight-1][randomWidth].getNeighbooringMines()+1);
        }
        if(randomHeight+1<heigthOfGame){
          model[randomHeight+1][randomWidth].setNeighbooringMines(model[randomHeight+1][randomWidth].getNeighbooringMines()+1);
        }
        if(randomWidth-1>=0 && randomHeight-1>=0){
          model[randomHeight-1][randomWidth-1].setNeighbooringMines(model[randomHeight-1][randomWidth-1].getNeighbooringMines()+1);
        }
